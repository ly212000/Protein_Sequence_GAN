...
def train_epoch(sess, trainable_model, num_iter,
    ...
    log_file = open('log_result.txt', 'a')
    generated_file = open('generated_sequences.txt', 'w')
    ...
    log_file.write('epoch statistics:\n')
    log_file.write('>>>> discriminator loss: ' + str(np.mean(d_losses)) + '\n')
    log_file.write('>>>> generator loss: ' + str(np.mean(supervised_g_losses)) + ' ' + str(np.mean(unsupervised_g_losses)) + '\n')
    ...
    if verify_sequence is not None:
        ...
        generated_file.write('>>>> correct generations (supervised, unsupervised): ' + str(np.mean(supervised_correct_generation)) + ' ' + str(np.mean(unsupervised_correct_generation)))
    ...
    generated_file.write('>>>> sampled generations (supervised, unsupervised):\n')
    generated_file.writelines([words[x] if words else x for x in supervised_gen_x] if supervised_gen_x is not None else str(None))
    generated_file.write('\n')
    generated_file.writelines([words[x] if words else x for x in unsupervised_gen_x] if unsupervised_gen_x is not None else str(None))
    generated_file.write('\n>>>> expected rewards: ' + str(np.mean(expected_rewards, axis=0)))
